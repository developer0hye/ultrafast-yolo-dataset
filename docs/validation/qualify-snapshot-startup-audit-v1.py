...
def main():
    ...
    parser.add_argument("--arguments", type=Path, required=True,
                        help="JSON object of explicit auditor arguments, excluding out")
    ...
    with tempfile.TemporaryDirectory(prefix="snapshot-audit-qualification-") as folder:
        ...
        reject("wrong source digest", lambda d: d["records"][0]["report"]["descriptor"]["library_sources"].update({"src/snapshot.rs": "0" * 64}))
        ...
        reject("overlapping intervals", lambda d: d["records"][1]["report"].update(wall_start_ns=d["records"][0]["report"]["wall_start_ns"]))
        ...
        reject("wrong first batch", lambda d: d["records"][0]["report"]["result"]["output_sha256"].update(first_batch="0" * 64))
        reject("negative stage", lambda d: d["records"][0]["report"]["result"]["stages_s"].update(_encode=-1))
        reject("stages exceed constructor", lambda d: d["records"][0]["report"]["result"]["stages_s"].update(_encode=1000))
        ...
        reject("RSS high water decreases", lambda d: d["records"][0]["report"]["result"].update(peak_rss_constructor_bytes=1))
        reject("RSS before exceeds high water", lambda d: d["records"][0]["report"]["result"].update(rss_before_constructor_bytes=10**15))
        reject("negative available memory", lambda d: d["records"][0]["report"]["result"].update(available_ram_bytes=-1))
        reject("negative CPU time", lambda d: d["records"][0]["report"]["result"].update(user_s=-1))
        reject("wrong cache payload", lambda d: d["records"][0]["report"]["cache"]["sections"][1].update(sha256="0" * 64))
        reject("wrong cache profile", lambda d: d["records"][0]["report"]["cache"].update(profile="0" * 64))
        reject("primer has measured result", lambda d: d["primers"][0].update(report=copy.deepcopy(d["records"][0]["report"])))
        ...
        for mode in ("miss", "hit"):
            for repeat in range(5):
                for version in (("baseline", "candidate") if repeat % 2 == 0 else ("candidate", "baseline")):
                    row = copy.deepcopy(next(r for r in original["records"] if r["mode"] == mode and r["version"] == version))
                    row["round"] = repeat
                    factor = ((1, 1.25, .8, 1.5, 1.1) if version == "baseline" else (1, 1.1, 1.3, .9, 1.2))[repeat]
        ...
        for index, row in enumerate(ordered):
            row["report"].update(pid=10000 + index, wall_start_ns=10**15 + index * 10**12,
                                 wall_end_ns=10**15 + index * 10**12 + 10**11)
        expanded["summary"] = {
            mode: {
                metric: auditor.summary([
                    [next(row["report"]["result"][metric] for row in expanded["records"]
                          if row["mode"] == mode and row["round"] == r and row["version"] == v)
                     for v in auditor.VERSIONS] for r in range(5)
        ...
        reject("wrong ratio", lambda d: d["summary"]["hit"]["constructor_s"].update(baseline_over_candidate=123), expanded)
        reject("wrong CI", lambda d: d["summary"]["miss"]["constructor_s"].update(paired_bootstrap_ci95=[0, 100]), expanded)
        reject("wrong summary raw pairs", lambda d: d["summary"]["hit"]["constructor_s"]["raw_pairs"][0].__setitem__(0, 1), expanded)
        control(copy.deepcopy(expanded), "synthetic five-round post-mutation control; not a measurement")

    receipt = {
        "complete": True, "controls": controls, "negative_cases": failures,
        "negative_case_count": len(failures), "auditor_sha256": auditor.sha(auditor_path),
