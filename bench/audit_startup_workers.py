...
HARNESS = (
    "bench/startup_worker_trial.py", "bench/cache_startup.py", "bench/cache_read_comparison.py",
    "bench/startup.py", "bench/fixture_files.py", "bench/label_engine.py", "tests/reference.py",
)
METRICS = (
    "constructor_s", "first_batch_total_s", "peak_rss_constructor_bytes",
    "peak_rss_first_batch_bytes", "user_s", "system_s",
)
...
def plan():
    ...
    for pair in range(5):
        rows.extend(dict(primer=False, pair=pair, position=i, workers=w)
...
def telemetry(path, rec, ram):
    ...
    return dict(
        samples=len(samples), sampled_whole_child_peak_rss_bytes=max(s["child_rss_bytes"] for s in samples),
...
def audit(args):
    parent, launch, identity, reference, fixture = map(read, (
        args.series, args.launch, args.identity, args.reference_audit, args.fixture_manifest,
    ))
    require(parent["complete"] is True and parent["passed"] is True and "error" not in parent, "unfinished/failed campaign")
    require((parent["planned_trials"], parent["planned_measured_trials"], parent["pairs"], parent["mode"])
            == (12, 10, 500000, "hit"), "campaign scope")
    ...
    require(parent["source_files"] == launch["source_files"] and set(parent["source_files"]) == set(HARNESS), "harness inventory")
    harness = {name: sha(args.source / name) for name in HARNESS}
    require(harness == parent["source_files"], "frozen harness bytes")
    for field, path in (("phase_audit_sha256", args.phase_audit),
    ...
    require(reference["complete_requested_samples"] is True and
            (reference["phase"], reference["task"], reference["pairs"]) == ("p3", "detect", 500000), "reference scope")
    require(reference["fixture_manifest_sha256"] == sha(args.fixture_manifest) and parent["fixture"] == fixture, "fixture anchor")
    require(fixture["count"] == 500000 and fixture["task"] == "detect" and
            fixture["fingerprint"]["files"] == 1000000, "fixture size/task")
    ...
    require(set(expected) == {"labels", "first_batch", "scan_summary_and_messages"} and
            all(CHECK.digest(v) for v in expected.values()), "output hash shape")
    require(identity["wheel_sha256"] == sha(args.wheel) and identity["toolchain"]["profile"] == "release", "qualified wheel")
    ...
    with ZipFile(args.wheel) as wheel:
        natives = [n for n in wheel.namelist() if n.startswith("ultrafast_yolo_dataset/_native.") and n.endswith((".so", ".pyd"))]
        require(len(natives) == 1 and hashlib.sha256(wheel.read(natives[0])).hexdigest() == identity["extension_sha256"], "wheel native bytes")
    ...
    require(parent["environment_overrides"] == dict(OMP_NUM_THREADS="1", OPENBLAS_NUM_THREADS="1", MKL_NUM_THREADS="1", YOLO_OFFLINE="true"), "thread environment")
    rows, artifacts = [], {}
    ...
    for index, (spec, rec) in enumerate(zip(plan(), parent["records"], strict=True)):
        ...
        require(rec["validated"] is True and type(rec["returncode"]) is int and rec["returncode"] == 0 and
                not rec.get("timed_out", False), "failed child")
        ...
        command = rec["command"]
        flags = ("corpus", "harness-root", "library-source", "wheel", "identity", "cache-root",
                 "expected-outputs", "out", "workers", "mode", "expected-cache-sha256")
        ...
        require(command[1] == str(Path(parent["source"]) / HARNESS[0]) and
                opts["harness-root"] == parent["source"], "worker invocation")
        require(opts["out"] == rec["path"] and opts["workers"] == str(spec["workers"]) and
                opts["mode"] == "hit" and opts["expected-cache-sha256"] == cache_sha, "trial arguments")
        ...
        for key, selected in (("corpus", args.fixture_manifest.parent), ("library-source", args.library_source),
                              ("wheel", args.wheel), ("identity", args.identity), ("expected-outputs", args.expected_outputs)):
        ...
        require(integer(raw["pid"], 1) == rec["pid"] and raw["mode"] == "hit" and
                integer(raw["workers"], 1) == spec["workers"], "raw identity/settings")
        rs, rf = integer(raw["started_at_ns"], 1), integer(raw["finished_at_ns"], 1)
        require(start <= rs < rf <= finish, "raw timestamps")
        require(raw["script_sha256"] == harness[HARNESS[0]] and
                raw["harness_sources"] == {k: v for k, v in harness.items() if k != HARNESS[0]}, "raw sources")
        ...
        require(descriptor["build_identity_sha256"] == sha(args.identity) and
                descriptor["harness_sha256"] == harness["bench/cache_read_comparison.py"], "runtime anchor")
        require(raw["cache"] == {k: cache[k] for k in ("bytes", "sha256", "sections")} |
                {"file": caches[spec["workers"]].name}, "cache container receipt")
        result = raw["result"]
        ...
        rows.append(dict(index=index, **spec, **{k: result[k] for k in METRICS},
                         stages_s=result["stages_s"], telemetry=observed))
    ...
    summary = {metric: CHECK.summary([[b[7][metric], b[16][metric]] for b in blocks])
               for metric in METRICS[:4]}
    return dict(
        passed=True, trials=12, measured_trials=10, pairs=500000, mode="hit", rows=rows, summary=summary,
        medians={str(w): {m: statistics.median(r[m] for r in measured if r["workers"] == w) for m in METRICS}
                 for w in (7, 16)}, descriptor=parent["descriptor"], artifacts=artifacts,
        series_sha256=sha(args.series), auditor_sha256=sha(Path(__file__)), helper_sha256=sha(HELPER),
        anchors={k: sha(getattr(args, k)) for k in ("launch", "controller", "identity", "reference_audit",
                 "fixture_manifest", "expected_outputs", "phase_audit", "pilot_qualification", "wheel")},
        scope="Five paired ratios of medians with all 5**5 paired bootstrap resamples; exploratory shared-host result. Primers excluded. Process high-water RSS includes imports/preflight; sampled whole-child telemetry does not isolate startup or bound unsampled peaks. Artifact audit trusts frozen worker checks for full input fingerprints and installed-runtime execution; it does not rerun the corpus or prove other hosts/tasks/cache-miss performance.",
    )


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    for name in ("series", "launch", "controller", "identity", "reference-audit", "fixture-manifest",
                 "expected-outputs", "phase-audit", "pilot-qualification", "wheel", "source", "library-source",
                 "original-cache", "cache-w7", "cache-w16", "runs", "out"):
    ...
    try:
        ...
        args.out.write_text(json.dumps(dict(passed=False, complete=True, error=traceback.format_exc()), indent=2) + "\n")
        raise
