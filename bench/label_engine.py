...
def prepare(root, task, count):
    ...
    for i in range(1000):
        ...
        for j in range(n):
            if task == "detect":
                values = rng.uniform(0.01, 0.99, 4)
            else:
                vertices = (3, 16, 100)[i % 3]
                values = rng.uniform(0.01, 0.99, vertices * 2)
    ...
    manifest = {
        ...
        "seed": 20260912,
        "bytes": total,
        "sha256_names_and_contents": digest.hexdigest(),
        "distinct_files": count,
        "templates": 1000,
        "objects_distribution_percent": {"1": 80, "10": 19, "100": 1},
        "segment_vertices_cycle": [3, 16, 100],
        "synthetic": True,
...
def reference(paths, workers):
    ...
    return {
        ...
        "object_offsets": np.asarray(offsets, np.int64),
        ...
        "segment_object_indices": np.asarray(objects, np.int64),
        "statuses": np.asarray(statuses, np.uint8),
