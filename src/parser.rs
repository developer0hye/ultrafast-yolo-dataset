...
impl Record {
    pub fn status(status: u8) -> Self { Self { status, ..Self::default() } }
}

fn number(s: &str) -> Option<f32> {
    if !s.bytes().all(|c| c.is_ascii_digit() || b".+-eE".contains(&c)) { return None; }
...
pub fn parse(text: &[u8], classes: usize, single_cls: bool) -> Record {
    let text = match std::str::from_utf8(text) {Ok(s) => s, Err(_) => return Record::status(3)};
    ...
    if !text.is_ascii() || text.bytes().any(|c| matches!(c, 11|12|28|29|30|133)) { return Record::status(3); }
    let lines: Vec<Vec<&str>> = text.trim().split(['\n','\r']).filter(|s| !s.is_empty())
        .map(|s| s.split_whitespace().collect()).collect();
    if lines.is_empty() { return Record::status(2); }
    let segment = lines.iter().any(|l| l.len()>6);
    if segment && lines.iter().any(|l| l.len()==5) { return Record::status(3); }
    let mut result = Record::default();
    for tokens in lines {
        if (!segment && tokens.len()!=5) || (segment && (tokens.len()<3 || tokens.len()%2==0)) {
            return Record::status(3);
        }
        let values: Option<Vec<f32>> = tokens.into_iter().map(number).collect();
        let values = match values {Some(v)=>v, None=>return Record::status(3)};
        let row = if segment {
            let points: Vec<[f32;2]> = values[1..].chunks_exact(2).map(|v|[v[0],v[1]]).collect();
            let mut lo=points[0]; let mut hi=points[0];
            for point in &points[1..] {
                for k in 0..2 {
                    // NumPy reductions choose the last equal signed zero.
                    if point[k]<=lo[k] {lo[k]=point[k];}
                    if point[k]>=hi[k] {hi[k]=point[k];}
                }
            }
            let row=[values[0], (lo[0]+hi[0])/2.0, (lo[1]+hi[1])/2.0, hi[0]-lo[0], hi[1]-lo[1]];
            result.segments.push(points);
            row
        } else { [values[0],values[1],values[2],values[3],values[4]] };
    ...
    if result.duplicates>0 {
        ...
        if segment {result.segments=indices.iter().map(|i|std::mem::take(&mut result.segments[*i])).collect();}
    }
    result
}
...
mod tests {
    ...
    fn conditional_order() {
        ...
        let b=b"2 .5 .5 .2 .2\n0 .4 .4 .1 .1\n2 .5 .5 .2 .2";
        let r=parse(b,3,false); assert_eq!(r.rows[0][0],0.0); assert_eq!(r.duplicates,1);
    }
    #[test]
    fn midpoint() { assert_eq!(number("1.0000000596046448").unwrap().to_bits(),1f32.to_bits()); }
    #[test]
    fn finite_and_utf8() {for s in [b"nan".as_slice(),b"\xff",b"1e999"] {assert_eq!(parse(s,2,false).status,3);}}
}
